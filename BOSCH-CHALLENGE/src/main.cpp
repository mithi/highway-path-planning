...
XYPoints start_engine(Vehicle& car, PathConverter& pathConverter){
  ...
  const double target_s = car.s + 0.5 * t * (car.v + target_speed);
...
int main() {
  ...
  PathConverter pathConverter("../data/highway_map_bosch1.csv", TRACK_DISTANCE);
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double car_s = j[1]["s"];
          double car_d = j[1]["d"];
          double car_speed = double(j[1]["speed"]) * mph_to_ms;
          ...
          if (just_starting) {
            ...
            XYPoints NextXY_points = pathConverter.make_path(
              trajectory.get_jmt_s(), trajectory.get_jmt_d(), TIME_INCREMENT, trajectory.n);
